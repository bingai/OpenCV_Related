...
int main()
{
  ...
  // Indicator for Corner Detection Algorithm
  bool success;
  ...
  for(int i{0}; i<images.size(); i++)
  {
    ...
    // Get image path and extension
    std::filesystem::path p(images[i]);
    ...
    */
    if(success)
    {
      ...
      // Draw the detected corner points on the checker board
      cv::drawChessboardCorners(frame, cv::Size(CHECKERBOARD[0],CHECKERBOARD[1]), corner_pts, success);

      cv::imwrite(path + imageName + "_corners.png", frame);
